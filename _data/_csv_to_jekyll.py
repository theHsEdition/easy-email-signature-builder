# Directions for how to use can be found in the Readme. Thats why you should Readit.
...
for row_index, row in enumerate(datareader):
	...
	# Othrwise, create a YAML file from the data in this row...
		...
		yaml_text = ""
		yaml_text += "---\n"
		yaml_text += "layout: summer-2017-template \n"
...
# set the destination for the MarkDown files
destination = "../_people/"

# move each file that is a Markdown file
for files in source:
	if files.endswith(".md"):
		#shutil.move(files,destination)
		shutil.move(os.path.join(files), os.path.join(destination, files))
